...
package io.rushstreet.connect.jdbc.util;
...
public class JdbcUtils {
  ...
  public static int getOracleDBTZOffset(Connection conn) throws SQLException, ConnectException {
	String query = "SELECT SIGN(TO_NUMBER(SUBSTR(TZ_OFFSET('US/Eastern'), 1, 3))) * (\n" + 
			"  TO_NUMBER(SUBSTR(TZ_OFFSET('US/Eastern'), 2, 2)) +\n" + 
			"  TO_NUMBER(SUBSTR(TZ_OFFSET('US/Eastern'), 5, 2)) / 60\n" + 
			") FROM DUAL";
	  
    try (Statement stmt = conn.createStatement()) {
      log.debug("executing query " + query + " to get current tz offset from database");
      ResultSet rs = stmt.executeQuery(query);
      if (rs.next()) {
        return rs.getInt(1);
      } else {
        throw new ConnectException(
            "Unable to get current tz offset from DB using query " + query + " on database"
        );
      }
    } catch (SQLException e) {
      log.error(
          "Failed to get current tz offset from DB using query " + query + " on database",
          e
      );
      throw e;
    }
  ...
  public static Timestamp getCurrentTimeOnDB(
    ...
    if ("Oracle".equals(dbProduct)) {
      query = "select LOCALTIMESTAMP from dual";
    } else if ("Apache Derby".equals(dbProduct)
        || (dbProduct != null && dbProduct.startsWith("DB2"))) {
      query = "values(LOCALTIMESTAMP)";
    } else {
      query = "select LOCALTIMESTAMP;";
    }
    
//    System.out.println("RSG-Debug");
//    ResultSet rs1 = conn.createStatement().executeQuery(query);
//    rs1.next();
//    System.out.println(rs1.getString(1));
//    ResultSet rs2 = conn.createStatement().executeQuery("select current_timestamp from dual");
//    rs2.next();
//    System.out.println(rs2.getString(1));
//    System.out.println(getOracleDBTZOffset(conn));
//    System.out.println("RSG-Debug-End");
    ...
    try (Statement stmt = conn.createStatement()) {
      ...
      if (rs.next()) {
    	if ("Oracle".equals(dbProduct)) {
    		return new Timestamp(rs.getTimestamp(1, cal).getTime() + getOracleDBTZOffset(conn) * 60 * 60 * 1000);
	    }
